...
// missing, returns null rather than a partial quote — never guess. The resolved
// text is entity-decoded (see decodeEntities) so every caller gets real
// characters, not raw "&#8217;"-style codes.
function lookup(ref) {
...
function injectVerses(text, atLineStart) {
  ...
  return text.replace(MARKER_RE, (_match, rawRef, offset, whole) => {
    ...
    // The ASV corpus contains no asterisk in any verse, so wrapping in *…* is
